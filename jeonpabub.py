...
def main():
    ...
    pdf_folder = "PDF_이미지"
    all_documents = []
    for root, _, _ in os.walk(pdf_folder):  # 모든 하위 폴더 탐색
        pdf_files = glob.glob(os.path.join(root, "*.pdf"))  # 현재 폴더 내의 PDF 파일 목록
        for pdf_file in pdf_files:
            pdf_path = save_uploadedfile(pdf_file)  # PDF 파일 저장 (필요한 경우)
            pdf_document = pdf_to_documents(pdf_path)  # PDF 변환
            smaller_documents = chunk_documents(pdf_document)  # 문서 조각화
            all_documents.extend(smaller_documents)  # 전체 문서 리스트에 추가
